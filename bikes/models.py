...
class Bike(models.Model):
    ...
    tube_fitting = models.CharField(max_length=1, choices=TubeFitting.choices, default='s')
    brake_type = models.CharField(max_length=10, choices=BrakeType.choices)

    def __str__(self):
        return ' '.join(str(i) for i in [self.make, self.model, self.serial_number] if i)

    class Meta:
        constraints = [
            models.UniqueConstraint(fields=('make', 'serial_number'), name='make-serial_number'),
        ]


class Repair(models.Model):
    bike = models.ForeignKey(Bike, models.PROTECT, related_name='repairs')
    user = models.ForeignKey('users.User', models.PROTECT, related_name='repairs_performed')
    time_started = models.DateTimeField(auto_now=False, auto_now_add=False, default=now)
    time_finished = models.DateTimeField(auto_now=False, auto_now_add=False, blank=True, null=True)
